function loadFlip() {
    ...
    function genPriceArray(item) {
        ...
        return [idToName(item), temp.b, temp.b - _temp.b, temp.s, temp.s - _temp.s];
    }
    ...
    for (let outputName of Object.keys(RECIPES)) {
        for (let recipe of RECIPES[outputName]) {
            ...
            let comment = ingredientsList.map(a => idToName(a) + " x " + recipe.i[a])
    ...
    sheet.getRange(startRow, 1, length, col).setValues(output)
    sheet.getRange(startRow, 1, length, 1).setNotes(comments)
    sheet.getRange(startRow, col+1, length, 1).setFormulaR1C1("=R[0]C[-2]-R[0]C[-9]");
    
    SpreadsheetApp.flush();
    sheet.getRange(startRow, 1, length, col+1).setBorder(false, false, false, false, false, false).sort({
        column: 11,
        ascending: false
    });
    
    SpreadsheetApp.flush();
    sheet.getRange(startRow + positive, 1, length, col+1).setBorder(true, false, false, false, false, false)/*.sort({
        column: 4,
        ascending: false
    });*/
